...
class NewsScraper :
    ...
    def _fetch_investing_br (self )->list [NewsItem ]:
        """
        Coleta notícias do Investing.com Brasil via scraping.

        DESABILITADO: Investing.com bloqueia scrapers com Cloudflare.
        Seletores CSS provavelmente desatualizados. Retorna lista vazia.

        Returns:
            Lista vazia (fonte desabilitada).
        """
        logger .debug (
        "Investing.com BR desabilitado — Cloudflare bloqueia scraping. "
        "Usando apenas Google News e InfoMoney."
        )
        return []
        ...
        try :
            ...
            if not articles_found :
                ...
                for link in all_links [:20 ]:
                    ...
                    full_url =(
                    f"https://br.investing.com{href }"
                    if href .startswith ("/")
                    else href
                    )

                    if not self ._seen_urls .add (full_url ):
                        continue
                    ...
                    url =full_url ,
                    published_at =datetime .now (tz =BRT ),
                    tickers_mentioned =tickers ,
                    )
                    )
            else :
                for article in articles_found [:20 ]:

                    title_elem =(
                    article .find ("a",class_ ="title")
                    or article .find ("a",{"data-test":"article-title-link"})
                    or article .find ("a")
                    )
                    ...
                    summary_elem =article .find ("p")or article .find ("div",class_ ="textDiv")
                    summary =(
                    summary_elem .get_text (strip =True )[:500 ]
                    if summary_elem
                    else ""
                    )
                    ...
                    pub_date =datetime .now (tz =BRT )
                    if date_elem :
                        date_text =date_elem .get ("datetime")or date_elem .get_text (strip =True )
                        if date_text :
                            pub_date =_parse_rss_date (date_text )
            ...
            logger .info ("Investing.com BR: %d notícias coletadas",len (items ))

        except ImportError :
            logger .warning (
            "beautifulsoup4 não instalado — scraping do Investing.com indisponível. "
            "Execute: pip install beautifulsoup4"
            )
        except Exception as exc :
            logger .warning ("Erro ao fazer scraping do Investing.com BR: %s",exc )
    ...
    def fetch_all_news (self )->list [NewsItem ]:
        ...
        Agrega resultados de Google News, InfoMoney e Investing.com BR.
        Se uma fonte falhar, as demais continuam sendo processadas.
        ...
        try :
            investing_items =self ._fetch_investing_br ()
            all_news .extend (investing_items )
        except Exception as exc :
            logger .warning ("Falha ao coletar Investing.com BR: %s",exc )

        all_news .sort (key =lambda n :n .published_at ,reverse =True )

        logger .info (
        "Total de notícias coletadas: %d (Google: %s, InfoMoney: %s, Investing: %s)",
        len (all_news ),
        sum (1 for n in all_news if n .source =="Google News"),
        sum (1 for n in all_news if n .source =="InfoMoney"),
        sum (1 for n in all_news if n .source =="Investing.com BR"),
        )
        return all_news
